...
final class Hpack {
    static final Map<C2015j, Integer> NAME_TO_FIRST_INDEX = nameToFirstIndex();
    ...
    static final class Reader {
        ...
        Reader(int i, int i2, C2000D d) {
            ...
            this.source = C2025t.m7756a(d);
        }

        Reader(int i, C2000D d) {
            this(i, i, d);
        }
        ...
        private void readLiteralHeaderWithIncrementalIndexingNewName() {
            C2015j readByteString = readByteString();
        ...
        /* access modifiers changed from: 0000 */
        public C2015j readByteString() {
            int readByte = readByte();
            boolean z = (readByte & 128) == 128;
            int readInt = readInt(readByte, Hpack.PREFIX_7_BITS);
            return z ? C2015j.m7719a(Huffman.get().decode(this.source.mo7122e((long) readInt))) : this.source.mo7105a((long) readInt);
        }

        /* access modifiers changed from: 0000 */
        public void readHeaders() {
            while (!this.source.mo7119d()) {
    ...
    static final class Writer {
        ...
        int nextHeaderIndex;
        private final C2011g out;
        private int smallestHeaderTableSizeSetting;
        private final boolean useCompression;

        Writer(int i, boolean z, C2011g gVar) {
        ...
        /* access modifiers changed from: 0000 */
        public void writeByteString(C2015j jVar) {
            int i;
            int i2;
            if (!this.useCompression || Huffman.get().encodedLength(jVar) >= jVar.mo7048g()) {
                i2 = jVar.mo7048g();
                i = 0;
            } else {
        ...
        public void writeHeaders(List<Header> list) {
            ...
            for (int i4 = 0; i4 < size; i4++) {
                ...
                C2015j h = header.name.mo7049h();
                C2015j jVar = header.value;
                Integer num = (Integer) Hpack.NAME_TO_FIRST_INDEX.get(h);
                if (num != null) {
                    i2 = num.intValue() + 1;
                    if (i2 > 1 && i2 < 8) {
                        if (Util.equal(Hpack.STATIC_HEADER_TABLE[i2 - 1].value, jVar)) {
                            i = i2;
                        } else if (Util.equal(Hpack.STATIC_HEADER_TABLE[i2].value, jVar)) {
                            i = i2;
                            i2++;
                        }
                    }
                ...
                if (i2 == -1) {
                    ...
                    while (true) {
                        ...
                        if (Util.equal(this.dynamicTable[i5].name, h)) {
                            if (Util.equal(this.dynamicTable[i5].value, jVar)) {
                ...
                if (i2 != -1) {
                    ...
                    if (i == -1) {
                        ...
                        writeByteString(h);
                    } else if (!h.mo7163b(Header.PSEUDO_PREFIX) || Header.TARGET_AUTHORITY.equals(h)) {
                        writeInt(i, 63, 64);
                    } else {
                        writeInt(i, 15, 0);
    ...
    static C2015j checkLowercase(C2015j jVar) {
        int g = jVar.mo7048g();
        int i = 0;
        while (i < g) {
            byte a = jVar.mo7036a(i);
            if (a < 65 || a > 90) {
                i++;
            } else {
                StringBuilder sb = new StringBuilder();
    ...
    private static Map<C2015j, Integer> nameToFirstIndex() {
